    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_EQUALS:  # '+' met of zonder shift
                    mb.update_iteraties(50)
...
class Mandelbrot:
    ...
    def update(self):
        if self.herteken:
            ...
            teken_mandelbrot(self.scherm, linksboven, rechtsonder, self.pallet, self.max_iter)
            pygame.display.flip()
            self.herteken = False
            print(f"Update duurde {time.time() - start:.2f} seconden")

    def update_iteraties(self, delta):
        self.max_iter += delta
        self.herteken = True
...
def teken_mandelbrot(scherm, linksboven, rechtsonder, pallet, max_iter):
    breedte = scherm.get_width()
    hoogte = scherm.get_height()
    pixel_array = pygame.surfarray.pixels2d(scherm)

    teken_mandelbrot_op_array(breedte, hoogte, linksboven, rechtsonder, pallet, max_iter, pixel_array)

    pygame.surfarray.blit_array(scherm, pixel_array)


@jit(nopython=True)
def teken_mandelbrot_op_array(breedte, hoogte, linksboven, rechtsonder, pallet, max_iter, pixel_array):
    for y in range(hoogte):
        ...
        for x in range(breedte):
            ...
            waarde = mandelbrot(positie, max_iter)
            kleur = rgb(0, 0, 0)
            if waarde >= 0:
